...
export default class BarGraph extends PureComponent {
  ...
  renderActiveBar = (reading, scale, options) => {
    const getTooltipMessage = reading => {
      ...
      return `Minified: ${parseFloat(formattedSize.size).toFixed(1)}${formattedSize.unit
        } | Gzipped: ${parseFloat(formattedGzip.size).toFixed(1)}${formattedGzip.unit
        }`
    }
      ...
      >
        <div className="bar-graph__bar-symbols">
          {options.isFirstTreeshakeable && (
            <div
              data-balloon={`ES2015 exports introduced. ${reading.hasSideEffects
          ...
          {options.isFirstSideEffectFree && (
            <div
              data-balloon={`Was marked side-effect free. ${reading.hasJSNext || reading.hasJSModule || reading.isModuleType
